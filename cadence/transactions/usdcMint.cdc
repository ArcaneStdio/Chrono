...
// This transaction mints new WrappedUSDC1 tokens and deposits them into a recipient's vault.
// It must be signed by the account that deployed the WrappedUSDC1 contract.

transaction(amount: UFix64, recipient: Address) {

    /// Reference to the Minter resource stored in the signer's account
    let minter: &WrappedUSDC1.Minter
    ...
    prepare(signer: auth(BorrowValue) &Account) {
        ...
        self.minter = signer.storage.borrow<&WrappedUSDC1.Minter>(from: WrappedUSDC1.MinterStoragePath)
        ...
        self.recipientReceiver = recipientAccount.capabilities.get<&{FungibleToken.Receiver}>(WrappedUSDC1.ReceiverPublicPath)
            .borrow()
    ...
    execute {
        ...
        log("Successfully minted and deposited ".concat(amount.toString()).concat(" WrappedUSDC1 to ").concat(recipient.toString()))
    }
}